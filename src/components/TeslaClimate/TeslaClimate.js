...
const TeslaClimate = props => (
  <div className="tesla-climate">
    <label className={`tesla-climate__item${props.value ? ' tesla-climate__item--active' : ''}${!props.limit ? ' tesla-heat' : ''}`} data-testid="tesla-climate">
      <p>{props.limit ? 'ac' : 'heat'} {props.value ? 'on' : 'off'}</p>
      <i className="tesla-climate__icon"></i>
      <input name="climate" type="checkbox" checked={props.value} onChange={() => {props.handleClimateChange()}} />
    </label>
  </div>
);
